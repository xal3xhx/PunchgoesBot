...
exports.run = async (client, message, [target, time, ...reason], level) => { // eslint-disable-line no-unused-vars
  ...
  if (level <= 3 && value >= 86500000) return message.reply("You can only tempmute for 1DAY or less.")
  ...
  await client.moderation("tempmute", by, username, user.id, reason, message, settings, unbanAt);
  mutedRole = message.guild.roles.cache.find(c => c.name === settings.mutedRole)
...
exports.help = {
  name: "tempmute",
  category: "moderation",
  description: "Adds the muted role to the user, time can be given in the 1D 1W 1Y etc format (1day 1week 1year)",
